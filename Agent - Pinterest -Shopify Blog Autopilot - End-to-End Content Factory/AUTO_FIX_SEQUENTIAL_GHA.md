...
### 4. **Flow đạt chuẩn META-PROMPT (Review → Cleanup → Featured image → Publish)**

- **queue-run** (1 bài): meta fix → gate 9/10 → nếu **gate pass**:
  1. Chạy **pre_publish_review** (scripts/pre_publish_review.py) cho bài đó.
  2. Nếu **review pass**:
     - **cleanup_before_publish** (strip generic, dedupe, set featured từ first inline, ưu tiên CDN).
     - **set_featured_image_if_missing** (nếu vẫn thiếu featured image → set từ first inline lần nữa).
