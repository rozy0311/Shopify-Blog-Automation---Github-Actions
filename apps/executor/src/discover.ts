...
type NicheIdeaResponse =
  | Array<{ topic: string; chars: number; searchScore?: number; engagementScore?: number; intentReason?: string }>
  | {
      ideas?: Array<{ topic: string; chars: number; searchScore?: number; engagementScore?: number; intentReason?: string }>;
      items?: Array<{ topic: string; chars: number; searchScore?: number; engagementScore?: number; intentReason?: string }>;
      topics?: Array<{ topic: string; chars: number; searchScore?: number; engagementScore?: number; intentReason?: string }>;
      results?: Array<{ topic: string; chars: number; searchScore?: number; engagementScore?: number; intentReason?: string }>;
    };
...
export async function generateNicheIdeas(
  ...
  const systemPrompt = [
    ...
    "",
      `Return a JSON object with exactly one key: {"ideas":[{"topic":"...","chars":N,"searchScore":1-10,"engagementScore":1-10,"intentReason":"..."}]}`,
    `The ideas array must contain exactly ${count} objects.`,
  ...
  const raw = await withRetry(() =>
      callStructuredLLM<NicheIdeaResponse>(
        systemPrompt,
        model,
        userPrompt,
      ),
  ...
  const rawIdeas = Array.isArray(raw)
    ? raw
    : raw.ideas || raw.items || raw.topics || raw.results || [];
